...
const fillPostsList = (posts) => {
  if (posts.length) {
    posts.forEach((post, index) => {
      ...
      postButtons.insertAdjacentHTML("afterbegin", `<button class='button__edit' onclick='editPost(${index})'>Редактировать`);
      postButtons.insertAdjacentHTML('beforeend', `<button class='button__delete' onclick='deletePost(${index})'>Удалить`);
    });
  }
}
